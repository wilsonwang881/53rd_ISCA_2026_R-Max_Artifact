...
void spp_l2::SPP_ORACLE::init() {
  ...
  std::cout << "Oracle L2: rollback " << (ROLLBACK_ENABLED ? "enabled." : "disabled.") << std::endl;
  file_read();
}
...
void spp_l2::SPP_ORACLE::file_write() {
  if (access.size() > 0) {
    ...
    rec_file.close();
    std::cout << "Oracle L2: writing " << access.size() << " accesses to file." << std::endl;
    access.clear();
  } 
}
...
uint64_t spp_l2::SPP_ORACLE::check_set_pf_avail(uint64_t addr) {
...
int spp_l2::SPP_ORACLE::check_pf_status(uint64_t addr) {
  uint64_t set = calc_set(addr); 
...
int spp_l2::SPP_ORACLE::update_pf_avail(uint64_t addr, uint64_t cycle) {
  addr = (addr >> 6) << 6;
  uint64_t set = calc_set(addr); 
...
std::vector<std::tuple<uint64_t, uint64_t, bool, bool>> spp_l2::SPP_ORACLE::poll(CACHE* cache) {
...
uint64_t spp_l2::SPP_ORACLE::rollback_prefetch(uint64_t addr) {
  uint64_t set = calc_set(addr); 
...
uint64_t spp_l2::SPP_ORACLE::calc_set(uint64_t addr) {
