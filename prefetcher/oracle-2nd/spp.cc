...
uint64_t spp_l2::prefetcher::issue(CACHE* cache) {
  uint64_t res = 0;
  ...
  if (!pending_pf_q.empty()) {
    ...
    if (!RFO_write && mshr_occupancy < cache->get_mshr_size() && ((cache->get_mshr_size() - mshr_occupancy) > erase_pos))  { 
      if (way == cache->NUM_WAY && search_mshr == cache->MSHR.end() && search_inflight_writes == cache->inflight_writes.end()) {
        ...
        pending_pf_q.erase(pending_pf_q.begin() + erase_pos);
        champsim::operable::lru_states_L2C.push_back(std::make_tuple(set, way, 1));
        goto restart;
...
void spp_l2::prefetcher::call_poll(CACHE* cache) {
...
void spp_l2::prefetcher::erase_duplicate_entry_in_ready_queue(CACHE* cache, uint64_t addr) {
  auto possible_duplicate_pf = std::find_if(std::begin(pending_pf_q), std::end(pending_pf_q),
                               [match = addr >> cache->OFFSET_BITS, shamt = cache->OFFSET_BITS](const auto& entry) {
                                 return (std::get<0>(entry) >> shamt) == match; 
                               });
...
void spp_l2::prefetcher::update_do_not_fill_queue(std::deque<uint64_t> &dq, uint64_t addr, bool erase, CACHE* cache, std::string q_name){
  uint64_t set = (addr >> 6) & champsim::bitmask(champsim::lg2(cache->NUM_SET));
...
void spp_l2::prefetcher::update_MSHR_inflight_write_rollback(CACHE* cache, SPP_ORACLE::acc_timestamp rollback_pf) {
  ...
  if (rollback_way < cache->NUM_WAY) 
    champsim::operable::lru_states_L2C.push_back(std::make_tuple(rollback_set, rollback_way, 0));

  // Update metric
  oracle.unhandled_misses_replaced++;
}

void spp_l2::prefetcher::place_rollback(CACHE* cache, std::deque<SPP_ORACLE::acc_timestamp>::iterator search, uint64_t set, uint64_t way) {
...
bool spp_l2::prefetcher::check_issued(CACHE* cache, uint64_t addr) {
...
bool spp_l2::prefetcher::search_MSHR_inflight_writes(CACHE* cache, Q_TYPE type, uint64_t addr) {
...
bool spp_l2::prefetcher::search_do_not_fill_qs(CACHE* cache, std::deque<uint64_t> q, uint64_t addr) {
  return std::find_if(std::begin(q), std::end(q),
         [match = addr >> cache->OFFSET_BITS, shamt = cache->OFFSET_BITS]
         (const auto& entry) {
           return (entry >> shamt) == match; 
...
void spp_l2::prefetcher::rollback_op(CACHE* cache, uint64_t addr, uint8_t type, std::deque<SPP_ORACLE::acc_timestamp>::iterator search, std::deque<SPP_ORACLE::acc_timestamp> &q, bool bkp_q, std::string debug_info) {
  uint64_t set = oracle.calc_set(addr);

  if (oracle.ROLLBACK_ENABLED){
    // Generate a rollback prefetch.
    spp_l2::SPP_ORACLE::acc_timestamp rollback_pf = rollback(addr, search, cache);
...
void spp_l2::prefetcher::cache_db_op(CACHE* cache, uint64_t addr, uint64_t set, uint8_t type, std::deque<SPP_ORACLE::acc_timestamp>::iterator search, std::deque<SPP_ORACLE::acc_timestamp> &q, bool bkp_q, std::string debug_info, bool &found_in_not_ready_queue, bool &found_in_pending_queue) {
